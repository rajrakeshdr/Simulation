...
If (WScript.Arguments.Count <> 1) Then
    ...
    WScript.Quit
Else
    Dim fso, svc, list, proc, startup, cfg, pid, str, cmd, query, dmp, dmp1, exfil
	...
	exfil = "C:\Simulate_Mitre\exfil\"
    dmp1 = fso.GetBaseName(str) & ".bin"
    ' if dump file already exists, try to remove it
    If(fso.FileExists(dmp1)) Then
      WScript.StdOut.WriteLine("Removing " & dmp1)
      fso.DeleteFile(dmp1)
    End If
    ...
    If(fso.FileExists(dmp1)) Then
      WScript.StdOut.WriteLine("Memory saved to " & dmp1)
    Else
